...
This example shows eight fonts. First use base graphics. `font_add` and `font_families` below are part of the `sysfonts` package loaded by `showtext`. Use these to load fonts and then check to see that they are now evailable, respectively. `showtext_auto` handles the rest. If you are not already familiar with `showtext` see its documentation for broader examples.
